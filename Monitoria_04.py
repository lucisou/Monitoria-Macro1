...
#gn_final = gn[n]  # Função política final do processo de iteração
#k_star_index = np.argmin(np.abs(k_grid - gn_final))  # Encontrar o índice onde k_grid é mais próximo ao gn_final
#print("Índice do capital estacionário:", k_star_index)  # Índice do capital estacionário



#g_ss = k_grid[k_star_index]
g_ss = k_grid[p]
print("Capital estacionário k* =", np.round(g_ss, 3))

# Consumo estacionário c* - a partir do k*
#c_ss = z * k_grid[k_star_index]**alpha + (1-delta)*k_grid[k_star_index] - k_grid[k_star_index]
